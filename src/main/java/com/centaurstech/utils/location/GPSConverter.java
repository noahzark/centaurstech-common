...
public class GPSConverter {
    ...
    public static GPSLocation gps84_To_Gcj02(double lat, double lon) {
        ...
        return new GPSLocation(mgLat, mgLon);
    }

    /**
     * * 火星坐标系 (GCJ-02) to 84 * * @param lon * @param lat * @return
     */
    public static GPSLocation gcj_To_Gps84(double lat, double lon) {
        GPSLocation gps = transform(lat, lon);
        double lontitude = lon * 2 - gps.getLng();
        double latitude = lat * 2 - gps.getLat();
        return new GPSLocation(latitude, lontitude);
    }
    ...
    public static GPSLocation gcj02_To_Bd09(double gg_lat, double gg_lon) {
        ...
        return new GPSLocation(bd_lat, bd_lon);
    }
     ...
     */
    public static GPSLocation bd09_To_Gcj02(double bd_lat, double bd_lon) {
        double x = bd_lon - 0.0065, y = bd_lat - 0.006;
        double z = Math.sqrt(x * x + y * y) - 0.00002 * Math.sin(y * pi);
        double theta = Math.atan2(y, x) - 0.000003 * Math.cos(x * pi);
        double gg_lon = z * Math.cos(theta);
        double gg_lat = z * Math.sin(theta);
        return new GPSLocation(gg_lat, gg_lon);
    }
     ...
     * @return
     */
    public static GPSLocation bd09_To_Gps84(double bd_lat, double bd_lon) {
        GPSLocation gcj02 = bd09_To_Gcj02(bd_lat, bd_lon);
        GPSLocation map84 = gcj_To_Gps84(gcj02.getLat(),
                gcj02.getLng());
        return map84;
    }
